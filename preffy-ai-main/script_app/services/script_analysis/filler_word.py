...
filler_word_system_prompt = get_prompt(
    service_name="script_analysis",
    function_name="filler_word",
    prompt_version=settings.FILLER_WORD_PROMPT_VERSION,
    prompt_type="system",
)
filler_word_user_prompt = get_prompt(
    service_name="script_analysis",
    function_name="filler_word",
    prompt_version=settings.FILLER_WORD_PROMPT_VERSION,
    prompt_type="user",
...
class FillerWordAnalyzer(SingleTurnScriptLLMWithSlidingWindow):
    def __init__(self):
        super().__init__(
            system_prompt=filler_word_system_prompt,
            user_prompt=filler_word_user_prompt,
            output_model=FillerWordAnalysisOutput,
            window_size=WINDOW_SIZE,
            window_overlap=WINDOW_OVERLAP,
        )
    
    async def get_filler_word(
        self, 
        script: List[ScriptSentence],
        metadata: Dict[str, Any],
    ) -> FillerWordAnalysisOutput:
        """
        Classify the speech acts in the given script.
        
        :param script: List of sentences in the script.
        :param metadata: Additional metadata for the classification.
        :return: SpeechActClassifierOutput containing classified speech acts.
        """
        output_with_dup: FillerWordAnalysisOutput = await self.do(
            script=script,
            metadata=metadata
        )
        ...
        for elem in output_with_dup:
            elem.filler_word = elem.filler_word.strip()
            if elem.filler_word not in filler_words:
                filler_words[elem.filler_word] = elem
            else:
                filler_words[elem.filler_word].indexes = list(set(
                    filler_words[elem.filler_word].indexes + elem.indexes
                ))
            
        return FillerWordAnalysisOutput(
            output=list(filler_words.values())
        )
...
if __name__ == "__main__":
    ...
    elements = asyncio.run(FillerWordAnalyzer().get_filler_word(
        script, metadata={}
    ))
    for element in elements.output:
        print(str(element.filler_word) + ' : ' + str(element.indexes))
        print("---")