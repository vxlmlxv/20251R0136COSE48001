...
speech_act_system_prompt = get_prompt(
    service_name="script_analysis",
    function_name="speech_act",
    prompt_version=settings.SPEECH_ACT_PROMPT_VERSION,
    prompt_type="system",
)
speech_act_user_prompt = get_prompt(
    service_name="script_analysis",
    function_name="speech_act",
    prompt_version=settings.SPEECH_ACT_PROMPT_VERSION,
    prompt_type="user",
...
class SpeechActAnalyzer(SingleTurnScriptLLMWithSlidingWindow):
    def __init__(self):
        super().__init__(
            system_prompt=speech_act_system_prompt,
            user_prompt=speech_act_user_prompt,
            output_model=SpeechActAnalysisOutput,
            window_size=WINDOW_SIZE,
            window_overlap=WINDOW_OVERLAP,
        )
    
    async def get_speech_act(
        self, 
        ...
        return SpeechActAnalysisOutput(
...
if __name__ == "__main__":
    ...
    elements = asyncio.run(SpeechActAnalyzer().get_speech_act(
        script, metadata={}
    ))
    for element in elements:
        print(str(element.index) + ' : ' + element.speech_act + " <- " + script[element.index].content)
        print("---")